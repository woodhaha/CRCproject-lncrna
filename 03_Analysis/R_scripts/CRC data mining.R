## DISCOVERING THE DIAGNOSTIC AND PROGNOSTIC LncRNA Biomarkers IN CRC PATIENTS USING MACHINE LEARNING APPROACHES. ##

# ====================================================================================================
# DEPRECATION WARNING
# ====================================================================================================
# This script is the ORIGINAL legacy version of the CRC data mining pipeline. It has been
# superseded by the refactored and optimized pipeline at:
#
#     ../pipeline.R
#
# KNOWN ISSUES IN THIS LEGACY SCRIPT:
#   1. doMC fork-based parallel backend does NOT work on Windows — `registerDoParallel(cores=3)`
#      with doMC silently falls back to sequential execution on Windows. Use doParallel +
#      makePSOCKcluster instead for cross-platform parallel support.
#   2. Cluster resource leak: `makeCluster(3)` is called INSIDE a `for(i in 0:10)` loop (lines ~380-388)
#      without a matching `stopCluster()` call inside the loop. Only one `stopCluster(cl)` exists
#      after the loop, which means each iteration creates a new 3-worker cluster without cleaning
#      up the previous one — draining system resources.
#   3. Hardcoded `setwd()` at line 24 — will fail if run outside the author's original directory.
#   4. Several `source()` calls reference helper scripts by relative path with no error handling.
#
# RECOMMENDATION: Use ../pipeline.R instead, which addresses all of these issues.
#
# This legacy script is preserved for reproducibility of the original analysis but is NOT
# recommended for new work. The deprecation banner does not affect script execution.
# ====================================================================================================
...
setwd("C:/Researching/CRC")
#load("E:/CRC_data_mining/Results/Documents.rdata")
par(mfrow = c(1, 1), omi = c(0.5, 0.5, 0.5, 0.5), las = 1)
...
#.libPaths("C:/R Working Directory/win-library/3.5")# Using R 3.5 and package library 
...
filter <- function(x) { length(x[x < 0.1]) / length(x) > 0.6 }
table(apply(expr, 1, filter)) ## 15255 remained.
##
expr <- expr[apply(expr, 1, filter) == FALSE,] #45243 genes removed.  
...
clinical <- clinical[!is.na(clinical$vital.status), ]
table.1 = table1(~OS + DFS + age + gender + preoperative.CEA + stage +
	 ...
	 overall=F,data=clinical) #+ location 
...
cairo_pdf(filename = "survival analysis versus stage.pdf", width = 6, height = 5)
plot.new()
...
Exprs <- as.data.frame(log2(as.matrix(Exprs) + 0.001)) #log2(FPMK+1) Normaliztion
...
table(apply(Exprs, 1, filter))

Exprs <- Exprs[apply(Exprs, 1, filter) == F,]


lncRNA_Exprs <- Exprs[rownames(Exprs) %in% lncRNA_v22$ENSG_id,]
## 799 lncRNA been Extracted using lncRNA_v22
...
par(mfrow = c(1, 4) ,mar = c(5, 2, 5, 2), las = 1)
...
parameter <- summary(fit, verbose = T) #summary_parameter
cairo_pdf(filename = "crossvalidation partial likehood deviance.pdf", width = 6, height = 5)
plot.new()
plot(parameter)
dev.off()
...
library(plotly)
deviance_plot = plotly::plot_ly(x = ~parameter$info$alpha, y = ~parameter$info$lambda, z = ~deviance) %>% add_surface(color = c("#FC4E07", "#E7B800", "#00AFBB"))
...
cv.glmfit <- cv.glmnet(y = Surv(survdata$OS, survdata$status), x = as.matrix(survdata[, 5:ncol(survdata)]),
			 family = "cox", parallel = T, standardize = T, alpha = parameter$opt.alpha, nlambda = 50)
#
stopCluster(cl)
#
par(mfrow = c(1, 1), mar = c(1, 1, 6, 1))
options(las=1)
cairo_pdf(filename = "Elastic Net Regularized Partial Likelihood Deviance.pdf", width = 6, height = 5)
plot.new()
plot(cv.glmfit, main="Elastic Net Regularized CoxPH Regression",xlab=expression(log(lambda)))
abline(h = parameter$opt.error, lty = 3)
dev.off()
...
cairo_pdf(filename = "Partial Likelihood Deviance.pdf", width = 6, height = 5)
plot.new()
...
cairo_pdf(filename = "Fraction Deviance Explained.pdf", width = 6, height = 5)
plot.new()
glmnet:::plotCoef(bet, lambda = res.cv$lambda, df = res.cv$df, dev = res.cv$dev.ratio,
...
abline(v = 0.059, lty = 3);abline(v = 0, lty = 3)
...
#
cairo_pdf(filename = "Features of candidate lncRNAs.pdf", width = 6, height = 5)
plot.new()
ggdotchart(ggtextdata, x = "Symbol", y = "logFC", dot.size = "HR", color = "HR.pvalue",
			...
			theme(plot.title = element_text(size = 18, hjust = 0.5, lineheight = 0.2),
...
clinical$group = ifelse(clinical$id %in% Train$id, "Training", "Testing")
clinical = clinical %>% dplyr::mutate(group = factor(group, levels = c("Training", "Testing", "P-value"), labels = c("Training", "Testing", "P-value")))
...
table.3 = table1(~OS  + age + status + gender + preoperative.CEA + stage  + lymphatic.invasion + perineural.invasion + venous.invasion | group,
				 topclass = "Rtable1-zebra", droplevels = F, render = rndr2, overall = F, data = clinical,
...
##Forestplot
cairo_pdf(filename = "Forestplot of coxph model.pdf", width = 6, height = 5)
plot.new()
...
library(survivalROC)
source("PlotsurvROC.R")
par(mfrow = c(1, 1))
cairo_pdf(filename = "1 year.pdf", width = 5, height = 5)
plot.new()
PlotsurvROC(1, Train, Test)
dev.off()
#
cairo_pdf(filename = "3 year.pdf", width = 5, height = 5)
plot.new()
PlotsurvROC(3,Train,Test)
dev.off()
#
cairo_pdf(filename = "5 year.pdf", width = 5, height = 5)
plot.new()
PlotsurvROC(5, Train, Test)
dev.off()
##
source("CALAUC.R")
pre_train = CALAUC_precompute(Train, cox_vip_list, stepcox)
pre_test  = CALAUC_precompute(Test,  cox_vip_list, stepcox)
...
cairo_pdf(filename = "multivaiable coxph survival predictions.pdf", width = 5, height = 5)
plot.new()
ggplot(AUC, aes(x = time, y = AUC, colour = group)) +
	geom_line(size = 1.2) + scale_color_manual(values = c("#FC4E07", "#0073C2FF")) +
	geom_rug(sides = "l") + labs(title = "Multivariable CoxPH model Risk predictions", y = "Area Under Curve (AUC)", x = "Observation Time (years)") +
	...
	geom_vline(xintercept = 7, linetype = "dashed", color = "grey", size = 0.5) +
	geom_vline(xintercept = 9, linetype = "dashed", color = "grey", size = 0.5) +
...
source("GGsurvplot.R")
cairo_pdf(filename = "coxph riskscore based survival predictions(Train).pdf", width = 5, height = 5)
plot.new()
GGsurvplot(cox_vip_list, Train, stepcox)
dev.off()
#
cairo_pdf(filename = "coxph riskscore based survival predictions(Test).pdf", width = 5, height = 5)
plot.new()
...
cairo_pdf(filename = "riskscores comparision.pdf", width = 5, height = 5)
plot.new()
...
par(mfrow = c(1, 1), mar = c(1, 1,1,1),las=2)
cairo_pdf(filename = "riskscores distributions.pdf", width = 6, height = 6)
plot.new()
ggdotchart(risk_df, x = "id", y = "log10.riskScore", shape = "group",
...
#
str(meregedata)
#colnames(meregedata)[8:14] =lookup(colnames(meregedata)[8:14], Annotation23[, -2])
...
source("rndr3.R")
table1(~OS + riskScore + risk + log10.riskScore + group + ENSG00000259065.1 + ENSG00000272913.1 + ENSG00000247095.2 + ENSG00000233223.2 + ENSG00000259974.2 + ENSG00000267317.2 + ENSG00000224272.2| survival.status, group, topclass = "Rtable1-zebra", data = meregedata, droplevels = F, render = rndr3, overall = F) #
...
#
source("PlotRFtune.R")
cairo_pdf(filename = "RF parameter tunning.pdf", width = 5, height = 5)
plot.new()
PlotRFtune(rf_tune)
dev.off()
##
...
#
cairo_pdf(filename = "RF variable importance.pdf", width = 7, height = 5)
plot.new()
...
cairo_pdf(filename = "Random forest predicted survival of patients in training data.pdf", width = 5, height = 5)
plot.new()
plot(gg_rfsrc(rfsrc)) + theme(legend.position = c(0.1, 0.2)) +
	labs(title = "Random forest predicted survival in training data", y = "Survival Probability", x = "Observation Time (years)") +
	...
	coord_cartesian(x = c(0, 4)) + theme_pubr() + geom_rug() +
 	annotate(geom = "text", x = 0.85, y = 0.5,
...
cairo_pdf(filename = "Random forest predicted survival of patients in testing data.pdf", width = 5, height = 5)
plot.new()
plot(gg_rfsrc(rf.pred)) + theme(legend.position = c(0.1, 0.2)) +
	labs(title = "Random forest predicted survival in testing data", y = "Survival Probability", x = "Observation Time (years)") +
...
cairo_pdf(filename = "Random forest variable importance.pdf", width = 5, height = 5)
plot.new()
plot(gg_vimp(rfsrc)) +
	theme(legend.position ="top") +
	labs(fill = "VIMP > 0", title = "Impotance of candidates") +	
...
#
cairo_pdf(filename = "Random forest predicted survival of stages.pdf", width = 5, height = 6)
plot.new()
...
cairo_pdf(filename = "Random forest predicted LncRNA Expression dependent Overall Survival Probability.pdf", width = 5, height = 5)
plot.new()
plot(gg_v, xvar = colnames(gg_v)[2:8],
...
par(mfrow = c(1, 1))
options(las=2)
cairo_pdf(filename = "Bier score(train).pdf", width = 5, height = 5)
plot.new()
...
lines(Bier_train$time, unlist(Bier_train$AppErr["Reference"]), "s", lwd = 1.5)
dev.off()
#
cairo_pdf(filename = "Bier score(test).pdf", width = 5, height = 5)
plot.new()
...
lines(Bier_test$time, unlist(Bier_test$AppErr["Reference"]), "s", lwd = 1.5)
dev.off()

#
cairo_pdf(filename = "Cindex (train).pdf", width = 5, height = 5)
plot.new()
...
dev.off()
#
cairo_pdf(filename = "Cindex(test).pdf", width = 5, height = 5)
plot.new()
...
Beta = Beta %>% gather(symbol, value, RP5.1021I20.1:AC114730.3) %>% mutate(lambda = log(lambda)) ##wide to length
# Plot
