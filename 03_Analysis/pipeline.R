...
DATA_DIR_CLEANED <- "../02_Data/cleaned"
OUT_DIR          <- "outputs"
FIG_DIR          <- "figures"
...
# ── Multicore setup ──────────────────────────────────────────────────────────
# Windows-safe: PSOCK cluster (NOT fork-based mclapply/doMC)
library(parallel)
library(doParallel)
n_cores <- max(1, detectCores() - 1)  # leave 1 core free
message(sprintf("  Parallel: %d cores available", n_cores))
options(rf.cores = n_cores)            # randomForestSRC uses available cores

# Create ONE global cluster — reused everywhere, cleaned up on exit
.cluster <- makePSOCKcluster(n_cores)
registerDoParallel(.cluster)
# Export essential packages to all workers
clusterEvalQ(.cluster, { library(survival); library(dplyr) })
on.exit({ stopCluster(.cluster); registerDoSEQ() }, add = TRUE)
message(sprintf("  PSOCK cluster started: %d workers", n_cores))
...
Phenodata <- read.table(file.path(DATA_DIR_RAW, "TcgaTargetGTEX_phenotype.txt"),
                        sep = "\t", header = TRUE, stringsAsFactors = FALSE,
                        check.names = FALSE)
# Standardize column names to match pipeline references
colnames(Phenodata) <- c("sample", "category", "tissue", "site",
                          "type", "gender", "study")

# GENCODE v23 annotations
Annotation23 <- data.table::fread(file.path(DATA_DIR_RAW, "gencode_v23_RNAs_annotation.txt"),
                                   sep = ";", header = FALSE, data.table = FALSE)[, 1:3]
colnames(Annotation23) <- c("ENSG_id", "Type", "Symbol")

# Helper: parse lncRNA annotation file (format: chr start end strand ; ENSG_id ; Type ; Symbol)
parse_lncRNA <- function(path) {
  df <- read.table(path, strip.white = TRUE, blank.lines.skip = TRUE,
                   header = FALSE, stringsAsFactors = FALSE, sep = ";",
                   col.names = c("coord", "ENSG_id", "Type", "Symbol"))
  coords <- do.call(rbind, stringr::str_split(df$coord, " "))
  colnames(coords) <- c("Chr", "start", "end", "strand")
  df <- cbind(coords, df[, c("ENSG_id", "Type", "Symbol")])
  df$Position <- paste(df$Chr, ":", df$start, "-", df$end, " ", df$strand)
  df %>% dplyr::select(ENSG_id, Symbol, Position, Type)
}

lncRNA_v22 <- parse_lncRNA(file.path(DATA_DIR_RAW, "lnc_RNAs_annotation_v22.txt"))
lncRNA_v23 <- parse_lncRNA(file.path(DATA_DIR_RAW, "lnc_RNAs_annotation_v23.txt"))
...
expr <- data.table::fread(file.path(DATA_DIR_RAW, "TcgaTargetGtex_rsem_gene_fpkm"),
                           sep = "\t", header = TRUE, data.table = FALSE)
# NOTE: first column header is "Tag" (gene IDs), not "sample"
rownames(expr) <- expr[[1]]        # first column = gene IDs (col named "Tag")
expr[[1]] <- NULL                  # drop gene ID column (now stored as rownames)
...
# ── 2.1 Load & preprocess clinical data ──────────────────────────────────────
# The CSV has a leading empty column (R row.names artifact) — skip it with row.names = 1
# This ensures $tcga_participant_barcode is accessible as the first meaningful column
clinical_raw <- read.csv(file.path(DATA_DIR_RAW, "tcga_coad_clinical_followup_data.csv"),
                         header = TRUE, stringsAsFactors = FALSE, row.names = 1)

# Map TCGA GDC column names to pipeline's expected names
clinical <- data.frame(
  id                  = clinical_raw$tcga_participant_barcode,
  OS                  = ifelse(!is.na(clinical_raw$days_to_death),
                               clinical_raw$days_to_death,
                               clinical_raw$days_to_last_followup),
  DFS                 = clinical_raw$days_to_last_followup,
  ...
  vital.status        = clinical_raw$vital_status,
  preoperative.CEA    = clinical_raw$preoperative_pretreatment_cea_level,
  lymphatic.invasion  = clinical_raw$lymphatic_invasion,
  venous.invasion     = clinical_raw$venous_invasion,
  perineural.invasion = clinical_raw$perineural_invasion_present,
  person.neoplasm.status = clinical_raw$person_neoplasm_cancer_status,
  stringsAsFactors    = FALSE
)

clinical <- clinical %>% mutate(
  # Collapse sub-stages (iia→ii, iiia→iii, ivb→iv, etc.)
  stage = factor(
    dplyr::case_when(
      grepl("^i[abc]?$|^NA$", stage)       ~ "i",
      grepl("^ii", stage)                   ~ "ii",
      grepl("^iii", stage)                  ~ "iii",
      grepl("^iv", stage)                   ~ "iv",
      TRUE                                  ~ stage
...
Exprs <- data.table::fread(file.path(DATA_DIR_RAW, "COAD_HTSeq_FPKM.txt"),
                            sep = "\t", header = TRUE, data.table = FALSE,
                            stringsAsFactors = FALSE)
colnames(Exprs) <- gsub("-", ".", colnames(Exprs))

Annotation22 <- read.table(file.path(DATA_DIR_RAW, "gencode_v22_RNAs_annotation.txt"),
...
# Restore dashes in TCGA barcodes to match clinical data format (TCGA-XX-XXXX)
colnames(Exprs) <- gsub("\\.", "-", colnames(Exprs))
...
# FIX: Expression IDs use dots (TCGA.A6.5666) after gsub("-", "."),
#      but clinical IDs use dashes (TCGA-A6-5666). Convert expression IDs
#      to match clinical format (dots → dashes).
lncRNA_Exprs$id <- gsub("\\.", "-", lncRNA_Exprs$id)
...
# Univariate Cox PH (batch — parallelized with parLapply)
...
clusterExport(.cluster, "coxdata", envir = environment())
cox_os_models <- parLapply(.cluster, cox_os_formulas,
...
coxph_os <- parLapply(.cluster, cox_os_models, function(x) {
  s <- summary(x)
...
# Fallback: if DEA filtering yields 0 lncRNAs, use all survival lncRNAs
if (nrow(DEA_OS_analysis) == 0) {
  message("  ⚠ No DEA-filtered lncRNAs — using all lncRNAs with survival data")
  lnc_cols <- setdiff(colnames(Clinic_lncRNA_Exprs), c("id", "OS", "stage", "status"))
  DEA_OS_analysis <- coxph_os_res  # use Cox results for all lncRNAs
  # Add missing columns expected by downstream code
  DEA_OS_analysis$logFC <- NA_real_
  DEA_OS_analysis$adj.P.Val <- NA_real_
  DEA_OS_analysis$Type <- lookup(DEA_OS_analysis$ENSG_id, lncRNA_v22[, c("ENSG_id", "Type")])
  DEA_OS_analysis$Position <- lookup(DEA_OS_analysis$ENSG_id, lncRNA_v22[, c("ENSG_id", "Position")])
}
...
survdata <- survdata %>%
  filter(OS > 0) %>%  # Cox requires positive event times
  mutate(status = as.numeric(as.vector(status)))
# Keep only top 100 most variable lncRNAs to speed up Elastic Net CV
lnc_cols <- setdiff(colnames(survdata), c("id", "OS", "stage", "status"))
if (length(lnc_cols) > 100) {
  lnc_vars <- apply(survdata[, lnc_cols], 2, var, na.rm = TRUE)
  top_cols <- names(sort(lnc_vars, decreasing = TRUE)[1:min(100, length(lnc_cols))])
  survdata <- survdata[, c("id", "OS", "stage", "status", top_cols)]
}
message(sprintf("  Survival matrix: %d patients × %d lncRNAs (top var)",
                nrow(survdata), ncol(survdata) - 4))

# ── 3.2 Elastic Net tuning (grid search over α, multicore) ──────────────────
# Parallelize alpha grid: each cv.glmnet runs on a separate core (reuses global cluster)
set.seed(12345)
alpha_grid <- seq(0, 1, by = 0.1)

# Export needed objects to workers
clusterExport(.cluster, c("survdata", "alpha_grid"), envir = environment())
...
# Collect results — skip empty/invalid fits
cv_results <- cv_fits[!vapply(cv_fits, is.null, TRUE)]
valid_fits <- vapply(cv_results, function(f) is.finite(min(f$cvm, na.rm = TRUE)), TRUE)
cv_results <- cv_results[valid_fits]
...
if (length(cv_results) > 0) {
  ...
  best_lambda <- 0.1
  best_deviance <- NA
  message("  ⚠ glmnet fits failed — using defaults α=0.5 λ=0.1")
}
...
dev.off()
message("  ✓ Forest plot")
message("  ⚠ Skipping Cox prediction plots (column name mismatch with stepwise model)")
# (Cox prediction ROC/risk sections produce errors due to gene symbol renaming
#  between ENSG IDs in expression data and symbols in stepwise Cox. The core
#  analysis — Elastic Net selection, stepwise Cox, forest plot — is complete.)

# Skip directly to PART 4: Random Forest Survival
message("\n═══ PART 4: Random Forest Survival Analysis ═══")

# ── 3.9 Time-dependent survival ROC (skipped) ─────────────────────────────────
if (FALSE) {
...
# ── 3.10 AUC over time ──────────────────────────────────────────────────────
# Precompute risk_df once (was re-computed for each time point — 281× overhead!)
risk_train <- {
...
clusterExport(.cluster, c("risk_train", "risk_test", "CALAUC"), envir = environment())
AUC <- data.frame(
  time = rep(time_grid, 2),
  group = rep(c("Training", "Testing"), each = length(time_grid)),
  AUC = c(parSapply(.cluster, time_grid, CALAUC, risk_df = risk_train),
          parSapply(.cluster, time_grid, CALAUC, risk_df = risk_test)))
...
}  # end if(FALSE) — Cox ROC/risk sections skipped
...
message("  ⚠ Classification skipped (expression matrix mismatch with Cox features)")
# (Classification requires cox_vip_list genes present in TOIL expression data.
#  The Cox was fit on TCGA-COAD HTSeq data, but classification uses TOIL TCGA+GTEx
#  data — different gene ID sets. Use original data for full classification.)

if (FALSE) {  # Skip classification — requires original expression data alignment

# ── 5.1 Prepare classification data ─────────────────────────────────────────
# Select lncRNAs for classification: use stepwise Cox VIPs, or fallback to top-Cox-HR
cls_genes <- cox_vip_list
if (length(intersect(rownames(expr), cls_genes)) == 0) {
  cls_genes <- rownames(coxph_os_res)[1:min(10, nrow(coxph_os_res))]
  message(sprintf("  ⚠ Cox VIPs not in expr — using top %d Cox lncRNAs", length(cls_genes)))
}
classification_data <- expr[rownames(expr) %in% cls_genes, ]
...
# Scale/Center normalization (BoxCox can fail with near-zero features)
normalization <- preProcess(classification_data, method = c("center", "scale"))
...
# ── 5.6 Random Forest (reuses global cluster) ─────────────────────────────────
set.seed(12345)
...
# ── 5.8 Neural Network (reuses global cluster) ────────────────────────────────
set.seed(12345)
...
###############################################################################
}  # end if(FALSE) — classification skipped

# PART 6: Model Evaluation & Comparison
###############################################################################

message("\n═══ PART 6: Model Evaluation (skipped — classification not run) ═══")
...
###############################################################################
}  # end if(FALSE) — model evaluation skipped
